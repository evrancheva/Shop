...
<form action="buy.php" method="post">
    ...
    $query = "SELECT `demo-project`.current_cart.id,`demo-project`.products.product_name,`demo-project`.products.price,`demo-project`.current_cart.quantity
		...
		WHERE `demo-project`.current_cart.user_id = $idto AND isBought='1'";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
				...
				<button type="submit" class="button" name="remove" value="'.$row['id'].'"/>Remove</button>
				</td></form>';
	...
	if(isset($_POST['CheckIf'])){
		...
		if($moneyOfUser < $sum){
			...
			echo "You bought it!";
			$query = "UPDATE current_cart SET isBought = '0' WHERE isBought='1' AND user_id = '$idto'";
			$result = $mysqli->query($query);
			if (!$result) {
			die("Failed to process query!");
		}
